# Attempt 7 Monitoring Checkpoint
$checkTime = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
Write-Output "=== CHECKPOINT: $checkTime ==="
Write-Output ""

# 1. Process alive?
$proc = Get-CimInstance Win32_Process -Filter "ProcessId = 17244" -ErrorAction SilentlyContinue
$worker = Get-CimInstance Win32_Process | Where-Object { $_.ParentProcessId -eq 17244 -and $_.Name -like "python*" }
if ($proc -and $worker) {
    Write-Output "✅ Merge process (Parent PID 17244 / Worker PID $($worker.ProcessId)) ALIVE"
} elseif ($proc) {
    Write-Output "✅ Parent process (PID 17244) ALIVE"
} else {
